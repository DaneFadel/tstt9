...
const int ShadowBufferWidth = 256;
const int ShadowBufferHeight = 256;
...
#endif //PBG_ADD_NEWCHAR_MONK_ANI
{
	...
	Speed *= (1.f / (FPS_ANIMATION_FACTOR / 1.f));
...
void BMD::AnimationTransformWithAttachHighModel_usingGlobalTM(OBJECT* oHighHierarchyModel, BMD* bmdHighHierarchyModel, int iBoneNumberHighHierarchyModel, vec3_t& vOutPosHighHiearachyModelBone, vec3_t* arrOutSetfAllBonePositions, bool bApplyTMtoVertices)
{
	...
	for (int i_ = 0; i_ < NumBones; ++i_)
	{
		...
		BoneTransform[i_][0][3] += v3Position[0];
		BoneTransform[i_][1][3] += v3Position[1];
		BoneTransform[i_][2][3] += v3Position[2];
...
void BMD::AnimationTransformOnlySelf(vec3_t* arrOutSetfAllBonePositions, const OBJECT* oSelf)
{
	...
	Animation(arrBonesTMLocal, oSelf->AnimationFrame, oSelf->PriorAnimationFrame, oSelf->PriorAction, const_cast<vec3_t&>(oSelf->Angle), Temp, false, true);
...
void BMD::AnimationTransformOnlySelf(vec3_t* arrOutSetfAllBonePositions, const vec3_t& v3Angle, const vec3_t& v3Position, const float& fScale, OBJECT* oRefAnimation, const float fFrameArea, const float fWeight)
{
	if (NumBones < 1)
		return;
	if (NumBones > MAX_BONES)
	...
	VectorCopy(v3Angle, v3RootAngle);
	VectorCopy(v3Position, v3RootPosition);
...
void BMD::Chrome(float* pchrome, int bone, vec3_t normal)
{
	...
	n = DotProduct(normal, chromerightvec);
	pchrome[0] = n + 1.f;

	n = DotProduct(normal, chromeupvec);
	pchrome[1] = n + 1.f;
}

void BMD::Lighting(vec3_t Light, Light_t* lp, vec3_t Position, vec3_t Normal)
{
	vec3_t LightDirection;
	VectorSubtract(lp->Position, Position, LightDirection);
	float Length = VectorLength(LightDirection);

	float LightCos = (DotProduct(Normal, LightDirection) / Length) * 0.8f + 0.3f;
	if (Length > lp->Range)
		LightCos -= (Length - lp->Range) * 0.01f;
	if (LightCos < 0.f)
		LightCos = 0.f;

	Light[0] += LightCos * lp->Color[0];
	Light[1] += LightCos * lp->Color[1];
	Light[2] += LightCos * lp->Color[2];
}
		...
		for (l = 0; l < MapWidth; l++)
		{
			...
			if (success == false)
			{
				...
				for (int c = 0; c < 3; c++)
				{
					...
					Bitmap[c] = Color > 255 ? 255 : static_cast<unsigned char>(Color);
				}
			}
		}
	}
...
{
	// Add your code here to create light maps
	// based on your desired logic and requirements.
}
...
void BMD::RenderMeshAlternative( int iRndExtFlag, int iParam, int i,int RenderFlag,float Alpha,int BlendMesh,float BlendMeshLight,float BlendMeshTexCoordU,float BlendMeshTexCoordV,int MeshTexture)
{
	...
	bool EnableWave = false;
	int streamMesh = StreamMesh;
	if (m->m_csTScript != NULL && m->m_csTScript->getStreamMesh())
	{
		streamMesh = i;
	}
	...
	if ((RenderFlag & RENDER_COLOR) == RENDER_COLOR)
	{
		...
		(RenderFlag & RENDER_OIL) == RENDER_OIL)
	    {
		if (m->m_csTScript != NULL && m->m_csTScript->getNoneBlendMesh())
			return;
		if (m->NoneBlendMesh)
			return;
	...
	if ((RenderFlag & RENDER_TEXTURE) == RENDER_TEXTURE)
	{
		Render = RENDER_TEXTURE;
		BindTexture(Texture);
		if ((RenderFlag & RENDER_BRIGHT) == RENDER_BRIGHT)
			...
			case RENDER_TEXTURE:
			{
				...
				if (EnableLight)
				{
					...
					if (Alpha >= 0.99f)
						...
						glColor4f(LightTransform[i][ni][0], LightTransform[i][ni][1], LightTransform[i][ni][2], Alpha);
				}
				break;
			}
			case RENDER_CHROME:
...
void BMD::RenderMeshTranslate(int i,int RenderFlag,float Alpha,int BlendMesh,float BlendMeshLight,float BlendMeshTexCoordU,float BlendMeshTexCoordV,int MeshTexture)
{
	...
	else if ((RenderFlag & (RENDER_CHROME | RENDER_METAL | RENDER_CHROME2 | RENDER_CHROME6)) != 0) 
	{
		if (m->m_csTScript != NULL) {
			if (m->m_csTScript->getNoneBlendMesh()) return;
		}
		if (m->NoneBlendMesh) return;
...
bool BMD::Open2(char* DirName, char* ModelFileName, bool bReAlloc)
{
	...
	for (i = 0; i < NumMeshs; i++)
	{
		...
		{
			m->m_csTScript = new TextureScript;
			*m->m_csTScript = static_cast<TextureScript&>(TSParsing);
		}
		else
		{
			delete m->m_csTScript;
	...
	for (i = 0; i < NumActions; i++)
	{
		Action_t* a = &Actions[i];
		a->Loop = false;
		a->NumAnimationKeys = *reinterpret_cast<short*>(Data + DataPtr);
		DataPtr += 2;
		a->LockPositions = *reinterpret_cast<bool*>(Data + DataPtr);
		DataPtr += 1;
		if (a->LockPositions)
		{
			a->Positions = new vec3_t[a->NumAnimationKeys];
			size_t Size = a->NumAnimationKeys * sizeof(vec3_t);
			memcpy(a->Positions, Data + DataPtr, Size);
			DataPtr += Size;
		}
		else
	...
	{
		Bone_t* b = &Bones[i];
		b->Dummy = *reinterpret_cast<char*>(Data + DataPtr);
		DataPtr += 1;
		if (!b->Dummy)
		{
			memcpy(b->Name, Data + DataPtr, 32);
			DataPtr += 32;
			b->Parent = *reinterpret_cast<short*>(Data + DataPtr);
			DataPtr += 2;
			b->BoneMatrixes = new BoneMatrix_t[NumActions];
			for (int j = 0; j < NumActions; j++)
			{
				BoneMatrix_t* bm = &b->BoneMatrixes[j];
				size_t Size = Actions[j].NumAnimationKeys * sizeof(vec3_t);
				int NumAnimationKeys = Actions[j].NumAnimationKeys;
				bm->Position = new vec3_t[NumAnimationKeys];
				bm->Rotation = new vec3_t[NumAnimationKeys];
				bm->Quaternion = new vec4_t[NumAnimationKeys];
...
bool BMD::Save2(char *DirName,char *ModelFileName)
{
	...
	{
		Action_t* a = &Actions[i];
		*((short*)pbyCur) = a->NumAnimationKeys; pbyCur += 2;
		*((bool*)pbyCur) = a->LockPositions; pbyCur += 1;
		if (a->LockPositions)
		{
	...
	{
		Bone_t* b = &Bones[i];
		*((char*)pbyCur) = b->Dummy; pbyCur += 1;
		if (!b->Dummy)
		{
			memcpy(pbyCur, b->Name, 32); pbyCur += 32;
			*((short*)pbyCur) = b->Parent; pbyCur += 2;
			for (int j = 0; j < NumActions; j++)
			{
				BoneMatrix_t* bm = &b->BoneMatrixes[j];
	...
	long lSize = static_cast<long>(pbyCur - pbyBuffer);
