...
void AttackEffect(CHARACTER *c)
{
	...
	case 77:
		if( ( c->Skill) == AT_SKILL_BOSS)
		{
			if((int)c->AttackTime == 2 || (int)c->AttackTime == 6)
			{
				vec3_t Angle = { 0.0f, 0.0f, 0.0f};
				int iCount = 40;
				for ( i = 0; i < iCount; ++i)
	...
	case 49:
		...
		if(( c->Skill) == AT_SKILL_BOSS)
		{
			if((int)c->AttackTime == 1)
			{
				...
				Luminosity = (15-(int)c->AttackTime)*0.1f;
...
bool CharacterAnimation(CHARACTER* c, OBJECT* o)
{
	...
	if(b->NumActions > 0)
	{
		PlaySpeed = b->Actions[b->CurrentAction].PlaySpeed * static_cast<float>(FPS_ANIMATION_FACTOR); 
		if(PlaySpeed < 0.f)
			PlaySpeed = 0.f;
...
bool AttackStage(CHARACTER* c, OBJECT* o)
{
	...
	case AT_SKILL_ONETOONE:
		{
			...
			if ( 2 <= (int)c->AttackTime && (int)c->AttackTime <= 8)
			{	// 기 모으기
				for ( int j = 0; j < 3; ++j)
				{
					vec3_t CurPos;
			...
			if ( 6 <= (int)c->AttackTime && (int)c->AttackTime <= 12)
			{	// 꼬깔 만들기
				vec3_t Position;
				...
				if(c->TargetCharacter != -1)
				{
					...
					{
						OBJECT *to = &tc->Object;
						if ( 10 <= (int)c->AttackTime && to->Live)
						{
							//PlayBuffer( SOUND_THUNDER01);
	...
	case AT_SKILL_SPEAR:	// 창찌르기
		{
			...
			if ( 13 <= (int)c->AttackTime && (int)c->AttackTime <= 14)
			{	// 현란한 창술
				for ( int i = 0; i < 3; ++i)
				{
					vec3_t Position;
    ...
    case AT_SKILL_REDUCEDEFENSE:
        if(o->Type == MODEL_PLAYER)
        {
            ...
            if((int)c->AttackTime >= 1 && (int)c->AttackTime <= 2 )
            {
                vec3_t Angle;
		...
		for(int i=0;i<2;i++)
		{
			...
			if ( w->Type!=-1 && w->Type!=MODEL_BOW+7 && w->Type!=MODEL_BOW+15 && w->Type!=MODEL_HELPER+5)
			{
				if(o->CurrentAction==PLAYER_ATTACK_BOW || o->CurrentAction==PLAYER_ATTACK_CROSSBOW || o->CurrentAction==PLAYER_ATTACK_FLY_BOW || o->CurrentAction==PLAYER_ATTACK_FLY_CROSSBOW)
				{
					...
					w->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_BOW].PlaySpeed * static_cast<float>(FPS_ANIMATION_FACTOR);
				}
				else if(w->Type==MODEL_MACE+2)
				{
					...
					{
						w->CurrentAction = 2;
						w->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_SWORD_RIGHT1].PlaySpeed * static_cast<float>(FPS_ANIMATION_FACTOR);
					}
					else
					{
						w->CurrentAction = 1;
						w->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_STOP_MALE].PlaySpeed * static_cast<float>(FPS_ANIMATION_FACTOR);
					}
				}
