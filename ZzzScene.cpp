...
char *szServerIpAddress = "181.215.236.22";
WORD g_ServerPort = 44410;
...
bool MoveMainCamera()
{
	...
	if (SceneFlag == MAIN_SCENE)
	{
		...
		if (Camera3DFov < -9.0f) Camera3DFov = -9.0f;
		if (Camera3DFov > 15.f) Camera3DFov = 15.f;
		if (CameraFOV < 16.f) CameraFOV = 16.f;
		if (CameraFOV > 48.f) CameraFOV = 48.f;
		
		if (Camera3DRoll)
		{
			if (SEASON3B::IsNone(VK_MBUTTON))
			{
				Camera3DRoll = false;
			}
		}
		else if (SEASON3B::IsRepeat(VK_MBUTTON) || SEASON3B::IsPress(VK_MBUTTON))
		{
			Camera3DRoll = true;
...
void MainScene(HDC hDC)
{
	...
	//for (int Remain = TimeRemain; Remain >= FRAME_DURATION; Remain -= FRAME_DURATION)
	//{
	...
	double timePerFrame = 1000 / REFERENCE_FPS;
