...
bool MoveHellasObjectSetting(int& objCount, int object)
{
    ...
    {
        LastAmbientSoundPlay = WorldTime;//if ( (MoveSceneFrame%100)==0 )
    ...
    if ( GetHellasLevel( Hero->Class, CharacterAttribute->Level )==KUNDUN_ZONE)
    {
        ...
        if ( ( CurrX>=25 && CurrY>=44 ) && ( CurrX<=51 && CurrY<=119 ) && (LastKundunSoundPlay < WorldTime - KundunSoundInterval) == 0) // MoveSceneFrame % 50
        {
...
bool AttackEffect_HellasMonster ( CHARACTER* c, CHARACTER* tc, OBJECT* o, OBJECT* to, BMD* b )
{
    ...
    case 277:
		...
		case AT_SKILL_BLOOD_ATT_UP+4:
        case AT_SKILL_REDUCEDEFENSE:
            CreateMonsterSkill_ReduceDef ( o, (int)c->AttackTime, 13, 200.f );
            break;

        case AT_SKILL_POISON:
            CreateMonsterSkill_Poison ( o, (int)c->AttackTime, 13 );
            break;

        case AT_SKILL_MONSTER_SUMMON:
            CreateMonsterSkill_Summon ( o, (int)c->AttackTime, 13 );
            break;
        ...
        if ( o->CurrentAction==MONSTER01_ATTACK2 && (int)c->AttackTime ==14 )
        {
    ...
    case 266:
    case 274:
        if ( o->CurrentAction==MONSTER01_ATTACK1 && (int)c->AttackTime >=13 )
        {
